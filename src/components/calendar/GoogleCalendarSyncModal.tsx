...
export function GoogleCalendarSyncModal({ isOpen, onClose }: GoogleCalendarSyncModalProps) {
  const { user, setGoogleCalendarConnected } = useAppStore();
  ...
  useEffect(() => {
    setIsConnected(user?.google_calendar_connected ?? false);
          ...
          <div className="space-y-4 text-xs">
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                ...
                className="w-full py-2 bg-white/[0.04] hover:bg-white/[0.08] text-zinc-300 font-medium rounded-xl border border-white/[0.08] text-[11px] transition-colors"
              >
                Conectar Instantâneo (Modo Operacional / Demonstração)
              </button>
            </div>
          </div>
        ) : (
